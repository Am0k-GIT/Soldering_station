...
bool ascendingABSsort (int16_t a, int16_t b)
{
    return abs(a) > abs(b);
}
...
{
	if (size > RING_BUF_MAX_SIZE) 
		size = RING_BUF_MAX_SIZE;
    Handle->_m_size = size;
	Handle->_m_index = 0;
    Handle->_m_full = false;
    Handle->_RW_array = malloc(size * sizeof(uint16_t));
    memset(Handle->_RW_array, 0, size * sizeof(uint16_t));
    Handle->_ERR_array = malloc(size * sizeof(uint16_t));
    memset(Handle->_ERR_array, 0, size * sizeof(uint16_t));
}

void RING_BUF_Push(RING_buffer_t* Handle, int16_t value)
{
    Handle->_RW_array[Handle->_m_index] = value;
...
int16_t RING_BUF_getFiltered(RING_buffer_t* Handle, uint8_t percent)
{
    ...
    {
        average += Handle->_RW_array[currentIndex];
    }
    average = round(average / size);

    for (uint8_t currentIndex = 0; currentIndex < size; currentIndex++)
    {
      Handle->_ERR_array[currentIndex] = average - Handle->_RW_array[currentIndex];
    }

    sortArray(Handle->_ERR_array, size, ascendingABSsort);
    ...
    {
        errors_sum += Handle->_ERR_array[currentIndex];
    }
